    ...
    while true do
        ...
        for i = 1, #Config.Doctor, 1 do
            ...
            if userDst <= 15 then
                sleep = 2
                if userDst <= 5 then
                    DrawText3D(konum.x, konum.y, konum.z, '[E] basarak estetik yaptır $'.. Config.doctorPrice)
                    ...
                    if userDst <= 1.5 then
                        ...
                        ESX.TriggerServerCallback('ali-npcdoctor:parakontrol', function(hasEnoughMoney)
                            if hasEnoughMoney then
                                TriggerServerEvent('ali-plasticsurgery:skin')
                                ESX.ShowNotification('Estetik operasyonu başlıyor!')
                                TriggerServerEvent('ali-npcdoctor:money')
                            else
                                ESX.ShowNotification('Yeteri kadar paran yok!')
                            end
                        end)
